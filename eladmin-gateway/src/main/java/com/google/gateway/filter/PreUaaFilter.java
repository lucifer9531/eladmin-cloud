...
import com.google.core.common.utils.TokenUtil;
...
public class PreUaaFilter implements GlobalFilter, Ordered {
	...
	private final ElAdminApiProperties elAdminApiProperties;

	private final RedisService redisService;

	/**
	 * 路径前缀以/eladmin开头，如eladmin-system
	 */
	public static final String PATH_PREFIX = "/eladmin";
	...
	public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
		// 如果未启用网关验证，则跳过
		if (!elAdminApiProperties.getEnable()) {
	...
	private boolean ignore(String path) {
		return elAdminApiProperties.getIgnoreUrl().stream()
				.map(url -> url.replace("/**", ""))
	...
	private Mono<Void> unauthorized(ServerHttpResponse resp, String msg) {
		return ResponseUtil.webFluxResponseWriter(resp, ElAdminConstant.JSON_UTF8, HttpStatus.UNAUTHORIZED, msg); }

	@Override
	public int getOrder() {
		return ElAdminConstant.EL_ADMIN_UAA_FILTER_ORDER;
	}
}
