package com.google.gateway.init;
...
import com.google.gateway.entity.GatewayRoute;
...
/**
 * 动态路由初始化类
 * 请将/doc/nacos目录下的eladmin-dynamic-routes.yaml配置至nacos
 * @author iris
 */
@Slf4j
@Configuration
...
public class DynamicRouteInit {
	...
	public void initRoute() {
		try {
			...
			String content = configService.getConfig(ElAdminConstant.CONFIG_DATA_ID_DYNAMIC_ROUTES, nacosProperties.getGroup(), ElAdminConstant.CONFIG_TIMEOUT_MS);
			log.info("初始化网关路由开始");
			updateRoute(content);
			log.info("初始化网关路由完成");
			//开户监听，实现动态
			configService.addListener(ElAdminConstant.CONFIG_DATA_ID_DYNAMIC_ROUTES, nacosProperties.getGroup(), new Listener() {
				@Override
