package com.google.core.web.listener;
...
public class RequestMappingScanListener implements ApplicationListener<ApplicationReadyEvent> {
	...
	public void onApplicationEvent(@NotNull ApplicationReadyEvent event) {
		...
		try {
			...
			res.put("list",list);
			redisService.hset(ElAdminConstant.EL_ADMIN_API_RESOURCE, microService, res, 18000L);
			redisService.sSetAndTime(ElAdminConstant.EL_ADMIN_SERVICE_RESOURCE, 18000L, microService);
			log.info("资源扫描结果:serviceId=[{}] size=[{}] redis缓存key=[{}]", microService,  list.size(), ElAdminConstant.EL_ADMIN_API_RESOURCE);
		} catch (Exception e) {
